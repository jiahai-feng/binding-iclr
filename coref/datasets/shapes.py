...
df = pd.read_csv(os.path.join(COREF_ROOT, "coref/datasets/raw/shapes.csv"))
shape_pairs = [(a, a) for a in df["shape"].to_list()]
...
class Vocab(BaseVocab):
    ...
    def __init__(self, tokenizer_type, split=None):
        ...
        self.filtered_names = self.filter_names(colors)
        self.filtered_country_capital_pairs = self.filter_countries(shape_pairs)
...
Answer: The {qn_subject} shape is the"""
        context_template = """ The {country} is {subject}."""
